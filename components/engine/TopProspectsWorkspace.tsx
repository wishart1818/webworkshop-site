...
const skipReasonLabels: Record<string, string> = {
  ...
  third_party_listing_only: "Third-party listing only",
  website_business_mismatch: "Business/domain mismatch",
  supplier_distributor: "Supplier/supply business",
  no_clear_local_service_intent: "No clear local service intent",
        ...
        <div className="engine-dialog-backdrop" onMouseDown={() => setOutreachResult(null)}>
          <dialog aria-labelledby="outreach-draft-title" className="engine-dialog engine-dialog--wide" onMouseDown={(event) => event.stopPropagation()} open>
            ...
            <div className="engine-package-dialog">
              ...
              <section><div className="engine-copy-head"><h3>Yes reply with preview link</h3><button className="engine-button" disabled={!outreachResult.emailQuality.ready} onClick={() => void copyText(`${outreachResult.id}:detailed`, outreachResult.prospect.outreach!.detailed)} type="button">{copied === `${outreachResult.id}:detailed` ? "Copied" : "Copy yes reply"}</button></div><pre>{outreachResult.prospect.outreach.detailed}</pre></section>
