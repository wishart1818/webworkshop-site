...
test("Top Prospects schema initializer creates only its additive tables under a transaction lock", async () => {
  ...
  assert.ok(WEBSITE_REPAIR_AUDIT_RUN_MIGRATION_STATEMENTS.every((statement) => fake.statements.includes(statement)));
  ...
  assert.ok(fake.statements.some((statement) => statement.includes(WEBSITE_REPAIR_AUDIT_RUN_MIGRATION_ID)));
...
test("Top Prospects schema initializer repairs migration bookkeeping and refuses partial schema", async () => {
  ...
  assert.ok(ready.statements.length > 58);
  ...
  assert.ok(ready.statements.some((statement) => statement.includes(WEBSITE_REPAIR_AUDIT_RUN_MIGRATION_ID)));
  ...
  assert.ok(WEBSITE_REPAIR_AUDIT_RUN_MIGRATION_STATEMENTS.every((statement) => ready.statements.includes(statement)));
